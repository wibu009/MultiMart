...
public class OAuthService : IScopedService
{
    ...
    public OAuthService(
        ...
        IStringLocalizer<OAuthService> t,
    ...
    public async Task<string> ExternalAuthAsync(string provider)
    {
        ...
        {
            "facebook" => _facebookOAuthService.GetLoginLinkUrl(state),
            "google" => _googleOAuthService.GetLoginLinkUrl(state),
    ...
    public async Task<string> ExternalAuthCallbackAsync(string provider, string code, string? error = "", string? state = "")
    {
        ...
        {
            "google" => await _googleOAuthService.GetUserFromCode(code),
            "facebook" => await _facebookOAuthService.GetUserFromCode(code),
            _ => throw new BadRequestException(_t["Invalid provider."])
        };
        ...
        await _userManager.AddToRoleAsync(customer, ApplicationRoles.Basic);

        await _cacheService.SetAsync(
            tokenCacheKey,
            Tuple.Create(customer.Id, clientIp),
            TimeSpan.FromMinutes(2));
        token = new StateData<string>(
                tenantId,
                tokenCacheKey,
        ...
        await _events.PublishAsync(new ApplicationUserCreatedEvent(customer.Id));
